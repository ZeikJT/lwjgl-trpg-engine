...
public class ModelData{
	...
	public void load(){
		try{
			...
			DataInputStream dbFile = new DataInputStream(new BufferedInputStream(new FileInputStream(this.file), 100));
			String obj = "OBJ";
			...
			for(int p=0; p<this.pieceCount; p++){
				if(this.hasTexture){
					textureName = "";
					// Fetch string length
			...
			int coff = 0;
			int toff = 0;
			for(int q=0; q<this.totalQuadCount*4; q++){
				voff = q*3;
				if(this.hasNormal){
